...
class PhraseTrigger(Trigger):
    def __init_(self,phrase):
        self.phrase=phrase
    def is_phrase_in(self,text):
        '''checks if phrase is in the text, return true if the text contains phrase and false otherwise'''
        ...
        x=phrase.split(' ')
        y=text.lower()
        y=''.join([ch if ch not in exclude else ' 'for ch in y])
        y=y.split(' ')
        ...
        for word in y:
            if word==x[0]:
                if y.index(word)>len(y)-len(x):
                    return False
                elif y[y.index(word):y.index(word)+len(x)]!=x:
                    y.remove(word)
                elif y[y.index(word):y.index(word)+len(x)]==x:
                    return True
        return False
...
class TitleTrigger(PhraseTrigger):
    ...
    def evaluate(self,story):
        '''
        returns true if the self.phrase attribute is in the title of the story'''
        x=story.get_title()
        return self.is_phrase_in(x)
...
def read_trigger_config(filename):
    ...
    for i in range(len(lines)):
        ...
        if trig[1]=='TITLE':
            ...
            trigger_dict[trig[0]]=AndTrigger(trig[2])
        elif trig[1]=='ADD':
            for y in range(1,len(trig)):
                trigger_list.append(trigger_dict[trig[y]])
